...
const Layout = ({
  ...
  return <div className="min-h-screen flex flex-col bg-gradient-to-br from-[#1a1500] via-[#302600] to-[#1a1500]">
      ...
      <header className="fixed top-0 left-0 w-full z-50">
            ...
            <div className="flex justify-between items-center py-4">
              <Link to="/" className="flex items-center space-x-2">
                <div className="h-10 w-10 rounded-full bg-[#5E4900] flex items-center justify-center">
                  <img src="./icon.png" alt="icon image"  className="w-full h-full rounded-full" />
                </div>
              ...
              <div className="md:hidden">
                <button type="button" className="text-gray-300 hover:text-white" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
        ...
        {/* Mobile Navigation */}
        {mobileMenuOpen && <div className="md:hidden backdrop-blur-xl bg-black/50 border-b border-[#5E4900]/30">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              {navLinks.map(link => <Link key={link.path} to={link.path} className={`block px-3 py-2 rounded-md text-base font-medium ${isActive(link.path) ? 'text-white bg-[#5E4900]/80' : 'text-gray-300 hover:text-white hover:bg-[#5E4900]/30'}`} onClick={() => setMobileMenuOpen(false)}>
                  {link.name}
                </Link>)}
            </div>
          </div>}
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <form action='https://formspree.io/f/xldwywqq' method='POST' className="flex">
                <input type="email" placeholder="Your email" name='Email-Address' required className="bg-black/30 backdrop-blur-sm text-white px-3 py-2 rounded-l-md border border-[#5E4900]/50 focus:outline-none focus:border-[#5E4900] flex-grow" />
                <button type="submit" className="bg-[#5E4900] hover:bg-[#6E5910] text-white px-3 py-2 rounded-r-md">
                  Send
                </button>
              </form>
            </div>
